...
class Options:
    def __init__(self):
        ...
        self.champions: List[BehavioralSpaceBoundingBox] = None
    ...
    def coord_parser(string: str):
        ...
        if len(data) != dim:
            raise argparse.ArgumentTypeError("Coordinate tuples should contain 4 values")
    ...
    def populate(p: argparse.ArgumentParser):
        ...
        group = p.add_argument_group("Extraction",
                                     "Retrieve specific data from the pickled file")
        group.add_argument('--champion', dest="champions", metavar='X0,Y0,X1,Y1',
                           type=Options.coord_parser, action='append',
                           help="Bounding box in the behavioral space from which the best performing individual,"
        ...
        group.add_argument('--fig-size', dest="fig_size", metavar='W,H',
                           type=Options.tuple_parser, help="Size of the generated plots")
...
def extract_champions(args, data, name):
    ...
    for c in args.champions:
        ...
        print("Best:", best)
        if best is not None:
            filename = args.dst_folder.joinpath(
                f"{name}_best_{best.features[0]:.2f}_{best.features[1]:.2f}.json")
            print(filename)
            best.to_file(filename)
            if filename.exists():
                logging.info(f"Saved best individual for {c} in {filename}")
            else:
                logging.warning(f"Failed to save best individual for {c} to {filename}")
        else:
